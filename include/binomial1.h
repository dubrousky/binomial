...
template<typename T> T binomial1<T>::operator()(T n, T k) {
    ...
    // enhance the multiply opration with unity
    LeftBinOpWithUnity<typename std::multiplies<T> > op((T)1);
    T denom = paccumulate(denominator.begin(),denominator.end(),op.unity(),op);
    T num = paccumulate(numerator.begin(),numerator.end(),op.unity(),op);
    assert(0 != denom);
    return num / denom;
}
